...
describe.skip('Resolve', () => {
  ...
  it('Should be able to resolve BindingType.Instance bindings', () => {
    ...
    expect(ninja.katana.handler instanceof KatanaHandler).toEqual(true);
    expect(ninja.katana.blade instanceof KatanaBlade).toEqual(true);
  ...
  it('Should store singleton type bindings in cache', () => {
    ...
    expect(katanaBinding?.cache === null).toEqual(true);
    expect(katanaBinding?.activated).toEqual(false);

    const ninja = resolveTyped<Ninja>(context);
    expect(ninja instanceof Ninja).toEqual(true);

    const ninja2 = resolveTyped<Ninja>(context);
    expect(ninja2 instanceof Ninja).toEqual(true);

    expect(katanaBinding?.cache instanceof Katana).toEqual(true);
    expect(katanaBinding?.activated).toEqual(true);
  ...
  it('Should throw when an invalid BindingType is detected', () => {
    ...
    expect(context.plan.rootRequest.bindings[0]?.type).toEqual(BindingTypeEnum.Invalid);
    expect(throwFunction).toThrow(`${ERROR_MSGS.INVALID_BINDING_TYPE} ${ninjaId}`);
  ...
  it('Should be able to resolve BindingType.DynamicValue bindings', () => {
    ...
    container.bind<Date>('Date').toDynamicValue((_context: interfaces.Context) => new Date());
  ...
  it('Should be able to resolve BindingType.Provider bindings', () => new Promise<void>(done => {
  ...
  it('Should be able to resolve plans with multi-injections', () => {
    ...
    expect(ninja2 instanceof Ninja).toEqual(true);
    expect(ninja2.katana instanceof Katana).toEqual(true);
  ...
  it('Should be able to resolve plans with async multi-injections', async () => {
    ...
    expect(ninja2 instanceof Ninja).toEqual(true);
    expect(ninja2.katana instanceof Katana).toEqual(true);
    expect(ninja2.shuriken === undefined).toEqual(true)
  ...
  it('Should be able to resolve plans with async and non async injections', async () => {
    ...
    expect(crazyInjectable.syncCtor).toEqual('syncCtor')
    expect(crazyInjectable.asyncCtor).toEqual('asyncCtor')
    expect(crazyInjectable.syncProperty).toEqual('syncProperty')
    expect(crazyInjectable.asyncProperty).toEqual('asyncProperty')
  ...
  it('Should be able to resolve plans with activation handlers', () => {
    ...
    container.bind<Katana>(katanaId).to(Katana).onActivation((_theContext: interfaces.Context, katana: Katana) => {
      const handler = {
    ...
    expect(ninja.katana.use()).toEqual('Used Katana!');
    expect(Array.isArray(timeTracker)).toEqual(true);
    expect(timeTracker.length).toEqual(2);
  ...
  it('Should be able to resolve BindingType.Function bindings', () => {
    ...
    expect(katanaFactoryBinding?.activated).toEqual(false);
    ...
    expect(ninja instanceof Ninja).toEqual(true);
    expect(typeof ninja.katanaFactory === 'function').toEqual(true);
    expect(ninja.katanaFactory() instanceof Katana).toEqual(true);
    expect(ninja.katanaFactory().handler instanceof KatanaHandler).toEqual(true);
    expect(ninja.katanaFactory().blade instanceof KatanaBlade).toEqual(true);
    expect(ninja.shuriken instanceof Shuriken).toEqual(true);
    expect(katanaFactoryBinding?.activated).toEqual(true);
  ...
  it('Should run the @PostConstruct method of parent class', () => {
    ...
    expect(ninja.katana.use()).toEqual('Used Weapon!');
  ...
  it('Should not cache bindings if a dependency in the async chain fails', async () => {
    try {
      ...
      container.bind('level1').toDynamicValue(async (_context) => {
        try {
          level1Attempts += 1;
      ...
      try {
        ...
        if (exception.name === 'first try failed.') {
          expect(level1Attempts).toEqual(1)
        }
        // ignore
      }

      const level2 = await container.getAsync<Level2>('a');
      expect(level2.value).toEqual('foobar');

      expect(level1Attempts).toEqual(2);
      expect(level2Attempts).toEqual(1);
    } catch (error) {
  ...
  it('Should only call parent async singleton once within child containers', async () => {
    ...
    expect(subject1 === subject2).toEqual(true);
  });

  it('should not deactivate a non activated constant value', () => {
    const container = new Container();
    const bindConstantValue = container
      .bind<string>('ConstantValue')
      .toConstantValue('Constant')
    const spy = vi.spyOn(bindConstantValue, "onDeactivation")
    bindConstantValue.onDeactivation(() => {});
  ...
  it('Should not allow transient construction with async preDestroy', async () => {
    ...
    expect(() => container.get('Destroyable')).toThrow('@preDestroy error in class Destroyable: Class cannot be instantiated in transient scope.');
  });
  ...
  it('Should not allow transient construction with async deactivation', async () => {
    ...
    expect(() => container.get('Destroyable')).toThrow('onDeactivation() error in class Destroyable: Class cannot be instantiated in transient scope.');
  });
  ...
  it('Should not allow request construction with preDestroy', async () => {
    ...
    expect(() => container.get('Destroyable')).toThrow('@preDestroy error in class Destroyable: Class cannot be instantiated in request scope.');
  });
  ...
  it('Should not allow request construction with deactivation', async () => {
    ...
    expect(() => container.get('Destroyable')).toThrow('onDeactivation() error in class Destroyable: Class cannot be instantiated in request scope.');
  });
  ...
  it('Should force a class with an async deactivation to use the async unbindAll api', async () => {
    ...
    expect(() => container.unbindAll()).toThrow('Attempting to unbind dependency with asynchronous destruction (@preDestroy or onDeactivation)');
  });
  ...
  it('Should force a class with an async deactivation to use the async unbind api', async () => {
    ...
    expect(() => container.unbind('Destroyable')).toThrow('Attempting to unbind dependency with asynchronous destruction (@preDestroy or onDeactivation)');
  });
  ...
  it('Should invoke destroy in order (all async): child container, parent container, binding, class', async () => {
    ...
    expect(binding).toEqual(3);
    expect(klass).toEqual(4);
  });
  ...
  it('Should force a class with an async onActivation to use the async api', async () => {
    ...
    expect(() => container.get('Constructable')).toThrow(`You are attempting to construct 'Constructable' in a synchronous way but it has asynchronous dependencies.`);
  });
  ...
  it('Should return resolved instance to onActivation when binding is async', async () => {
    ...
    container.bind<Constructable>('Constructable').toDynamicValue(() => Promise.resolve(new Constructable())).inSingletonScope()
      .onActivation((_context, c) => new Promise((r) => {
        activated = c
        r(c);
      }));
  ...
  it('Should not allow sync get if an async activation was added to container', async () => {
    ...
    expect(() => container.get('foo')).toThrow(`You are attempting to construct 'foo' in a synchronous way but it has asynchronous dependencies.`);
  });
  ...
  it('Should allow onActivation to replace objects in async autoBindInjectable chain', async () => {
    ...
    expect(level2).toEqual(constructedLevel2);

    const level3 = await container.getAsync(Level3);

    expect(level3.level2).toEqual(constructedLevel2);
  });
  ...
  it('Should allow onActivation (sync) of a previously binded sync object (with activation)', async () => {
    ...
    container.onActivation('foo', (_context, previous) => `${previous}baz`);

    const result = container.get('foo');

    expect(result).toEqual('bumbaz');
  });
  ...
  it('Should allow onActivation (async) of a previously binded sync object (with activation)', async () => {
    ...
    container.onActivation('foo', (_context, previous) => Promise.resolve(`${previous}baz`));
  ...
  it('Should allow onActivation (sync) of parent (async) through autobind tree', async () => {
    ...
    expect(result.parent).toEqual(constructed);
  });
  ...
  it('Should priortize onActivation of parent container over child container', () => {
    const container = new Container();
    container.onActivation('foo', (_context, previous) => `${previous}baz`);
    container.onActivation('foo', (_context, previous) => `${previous}1`);

    const child = container.createChild();

    child.bind<string>('foo').toConstantValue('bar').onActivation((_c, previous) => `${previous}bah`);
    child.onActivation('foo', (_context, previous) => `${previous}bum`);
    child.onActivation('foo', (_context, previous) => `${previous}2`);

    const result = child.get('foo');

    expect(result).toEqual('barbahbaz1bum2');
  });

  it('Should priortize async onActivation of parent container over child container (async)', async () => {
    const container = new Container();
    container.onActivation('foo', async (_context, previous) => `${previous}baz`);
    container.onActivation('foo', async (_context, previous) => `${previous}1`);

    const child = container.createChild();

    child.bind<string>('foo').toConstantValue('bar').onActivation((_c, previous) => `${previous}bah`);
    child.onActivation('foo', async (_context, previous) => `${previous}bum`);
    child.onActivation('foo', async (_context, previous) => `${previous}2`);
  ...
  it('Should not allow onActivation of parent on child container', async () => {
    ...
    expect(result.parent).toStrictEqual(constructed);
  });
  ...
  it('Should only call async method once if marked as singleton (indirect)', async () => {
    ...
    expect(subject1.doSomething() === subject2.doSomething()).toEqual(true);
  });
  ...
  it('Should support shared async singletons when using autoBindInjectable', async () => {
    ...
    expect(async).toEqual(object1.asyncValue);
  });
  ...
  it('Should be able to mix async bindings with non-async values', async () => {
    ...
    const subject1 = await container.getAsync<UseDate>('UseDate');
    expect(subject1.foobar).toEqual('foobar');
  });
  ...
  it('Should throw exception if using sync API with async dependencies', async () => {
    ...
    expect(() => container.get<UseDate>('UseDate')).toThrow(`You are attempting to construct 'UseDate' in a synchronous way but it has asynchronous dependencies.`);
  });
  ...
  it('Should error if trying to resolve an promise in sync API', () => {
    ...
    expect(() => container.get<string>('async')).toThrow(`You are attempting to construct 'async' in a synchronous way but it has asynchronous dependencies.`);
  });
  ...
  it('Should cache a a resolved value on singleton when possible', async () => {
    ...
    const asyncServiceDynamicValueCallback = vi.fn(() => asyncServiceDynamicValue);
    ...
    expect(asyncServiceDynamicValueCallback).toHaveBeenCalled();
    expect(serviceFromGetAsync).toEqual(asyncServiceDynamicResolvedValue);
    expect(serviceFromGet).toEqual(asyncServiceDynamicResolvedValue);
  });
});