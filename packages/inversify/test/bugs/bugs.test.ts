...
describe('Bugs', () => {
  ...
  it('Should not throw when args length of base and derived class match (property setter)', () => {
    ...
    expect(master.rank).toEqual('master');
  ...
  it('Should be able to convert a Symbol value to a string', () => {
    ...
    expect(throwF).toThrow(`${ERROR_MSGS.NOT_REGISTERED} ${getServiceIdentifierAsString(TYPES.Weapon)}`);
  ...
  it('Should be able to combine dynamic value with singleton scope', () => {
    ...
    container.bind<number>('transient_random').toDynamicValue(() =>
      Math.random()).inTransientScope();

    container.bind<number>('singleton_random').toDynamicValue(() =>
  ...
  it('Should be able to use an abstract class as the serviceIdentifier', () => {
    ...
    expect(jungle.animal.makeSound('zzz')).toEqual('ssssszzz');
    expect(jungle.animal.move(5)).toEqual('Slithering... Snake moved 5m');
      ...
      return (args: interfaces.NextArgs) => {
        ...
        args.contextInterceptor = (context: interfaces.Context) => {
          ...
          context.plan.rootRequest.childRequests.forEach((request, index) => {
            ...
            switch (index) {
              case 0:
                expect(request.target.isNamed()).toEqual(false);
                expect(request.target.isTagged()).toEqual(false);
                break;
              case 1:
                expect(request.target.isNamed()).toEqual(false);
                expect(request.target.isTagged()).toEqual(false);
                break;
              case 2:
                expect(request.target.isNamed()).toEqual(true);
                expect(request.target.isTagged()).toEqual(false);
                break;
              case 3:
                expect(request.target.isNamed()).toEqual(false);
                expect(request.target.isTagged()).toEqual(true);
                break;
              case 4:
                expect(request.target.isNamed()).toEqual(false);
                expect(request.target.isTagged()).toEqual(false);
  ...
  it('Helper getFunctionName should not throw when using an anonymous function', () => {
    ...
    expect(name).toEqual('Anonymous function: ' + (function (options: unknown) {
      return options;
    }).toString());
  ...
  it('Should be able to get all the available bindings for a service identifier', () => {
    ...
    expect(appControllerNamedRight.length).toEqual(1);
    expect(appControllerNamedRight[0]?.name).toEqual('AppController');
    ...
    expect(appControllerTaggedRight.length).toEqual(1);
    expect(appControllerTaggedRight[0]?.name).toEqual('AppController2');

    const getAppController = () => {
      const matches = container.getAll<Controller>(controllerId);
      expect(matches.length).toEqual(2);
      expect(matches[0]?.name).toEqual('AppController');
      expect(matches[1]?.name).toEqual('AppController2');
    };
  ...
  it('Should generate correct metadata when the spread operator is used', () => {
    ...
    const serviceIdentifiers = getMetadata<any>(METADATA_KEY.TAGGED, Foo);
    expect(serviceIdentifiers['0'][0].value.toString()).toEqual('Symbol(BAR)');

    // is the plan correct?
    const dependencies = getDependencies(new MetadataReader(), Foo);
    expect(dependencies.length).toEqual(1);
    expect(dependencies[0]?.serviceIdentifier.toString()).toEqual('Symbol(BAR)');
    ...
    expect(foo.bar.length).toEqual(2);
    expect(foo.bar[0]?.name).toEqual('bar1');
    expect(foo.bar[1]?.name).toEqual('bar2');
  ...
  it('Should be able to inject into an abstract class', () => {
    ...
    expect(soldier.weapon instanceof DefaultWeapon).toEqual(true);
    expect(knight.weapon instanceof Sword).toEqual(true);
    expect(archer.weapon instanceof Bow).toEqual(true);
  ...
  it('Should be able apply inject to property shortcut', () => {
    ...
    expect(ninja.fight()).toEqual('Used Katana!');
  ...
  it('Should be able to combine unmanaged and managed injections ', () => {
    ...
    class TypedBL extends BLBase<Type> {
      public constructor(
        @inject(TypedRepo) repository: TypedRepo
      ) {
    ...
    expect(typedBL.repository.model.instance.name).toEqual(new Type().name);
  ...
  it('Should detect missing annotations in base classes', () => {
    ...
    expect(tryGet).toThrow('Missing required @injectable annotation in: Warrior.');
