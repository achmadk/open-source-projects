...
describe('Lookup', () => {
  ...
  it('Should throw when invoking get, remove or hasKey with a null key', () => {
    ...
    expect(() => { lookup.get(invalid); }).toThrow(ERROR_MSGS.NULL_ARGUMENT);
    expect(() => { lookup.remove(invalid); }).toThrow(ERROR_MSGS.NULL_ARGUMENT);
    expect(() => { lookup.hasKey(invalid); }).toThrow(ERROR_MSGS.NULL_ARGUMENT);
  });

  it('Should throw when attempting to add a null key', () => {
    const lookup = new Lookup<unknown>();
    expect(() => { lookup.add(invalid, new ClonableValue<number>(1)); }).toThrow(ERROR_MSGS.NULL_ARGUMENT);
  });

  it('Should throw when attempting to add a null value', () => {
    const lookup = new Lookup<unknown>();
    expect(() => { lookup.add('TEST_KEY', null); }).toThrow(ERROR_MSGS.NULL_ARGUMENT);
  });
  ...
  it('Should throws when key not found', () => {
    const lookup = new Lookup<unknown>();
    expect(() => { lookup.get('THIS_KEY_IS_NOT_AVAILABLE'); }).toThrow(ERROR_MSGS.KEY_NOT_FOUND);
    expect(() => { lookup.remove('THIS_KEY_IS_NOT_AVAILABLE'); }).toThrow(ERROR_MSGS.KEY_NOT_FOUND);
  });
  ...
  it('Should be clonable', () => {
    ...
    expect(copy.hasKey(key1)).toEqual(true);
  ...
  it('Should use use the original non clonable entry if it is not clonable', () => {
    ...
    const copy = lookup.clone();
    expect(copy.get(key1)[0] === warrior).toEqual(true);
  ...
  it('Should be able to remove a binding by a condition', () => {
    ...
    expect(lookup1.hasKey(warriorId)).toEqual(true);
    expect(lookup1.hasKey(weaponId)).toEqual(true);
    expect(lookup1.get(warriorId).length).toEqual(2);
    expect(lookup1.get(weaponId).length).toEqual(2);

    const removeByModule1 = removeByModule(moduleId1);
    lookup1.removeByCondition(removeByModule1);
    expect(lookup1.hasKey(warriorId)).toEqual(true);
    expect(lookup1.hasKey(weaponId)).toEqual(true);
    expect(lookup1.get(warriorId).length).toEqual(1);
    expect(lookup1.get(weaponId).length).toEqual(1);

    const lookup2 = getLookup();
    expect(lookup2.hasKey(warriorId)).toEqual(true);
    expect(lookup2.hasKey(weaponId)).toEqual(true);
    expect(lookup2.get(warriorId).length).toEqual(2);
    expect(lookup2.get(weaponId).length).toEqual(2);
    ...
    expect(lookup2.hasKey(warriorId)).toEqual(false);
    expect(lookup2.hasKey(weaponId)).toEqual(false);
  ...
  it('should be able to remove the intersection with another lookup', () => {
    ...
    expect(lookup.getMap()).toStrictEqual(new Map([
