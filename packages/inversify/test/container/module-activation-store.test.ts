...
describe('ModuleActivationStore', () => {
  it('should remove handlers added by the module', () => {
    ...
    const onActivation1: interfaces.BindingActivation<unknown> = (_c, a) => a;
    const onActivation2: interfaces.BindingActivation<unknown> = (_c, a) => a;
    const onActivation3: interfaces.BindingActivation<unknown> = (_c, a) => a;
    const onDeactivation1: interfaces.BindingDeactivation<unknown> = () => Promise.resolve();
    const onDeactivation2: interfaces.BindingDeactivation<unknown> = () => Promise.resolve();
    const onDeactivation3: interfaces.BindingDeactivation<unknown> = () => Promise.resolve();
    ...
    const onActivationMod2: interfaces.BindingActivation<unknown> = (_c, a) => a;
    const onDeactivationMod2: interfaces.BindingDeactivation<unknown> = () => Promise.resolve();
    ...
    expect(handlers.onActivations.getMap()).toStrictEqual(new Map([
      [serviceIdentifier1, [onActivation1, onActivation2]],
      [serviceIdentifier2, [onActivation3]]
    ]));
    expect(handlers.onDeactivations.getMap()).toStrictEqual(new Map([
    ...
    expect(noHandlers.onActivations.getMap()).toStrictEqual(new Map());
    expect(noHandlers.onDeactivations.getMap()).toStrictEqual(new Map());

    const module2Handlers = moduleActivationStore.remove(moduleId2);
    expect(module2Handlers.onActivations.getMap()).toStrictEqual(new Map([[serviceIdentifier1, [onActivationMod2]]]));
    expect(module2Handlers.onDeactivations.getMap()).toStrictEqual(new Map([[serviceIdentifier1, [onDeactivationMod2]]]));
  });
  ...
  it('should be able to clone', () => {
    ...
    //change original
    const onActivation4: interfaces.BindingActivation<unknown> = (_c, a) => a;
    const onDeactivation4: interfaces.BindingDeactivation<unknown> = () => Promise.resolve();
    ...
    expect(cloneModule1Handlers.onActivations.getMap()).toStrictEqual(
      new Map([
        [serviceIdentifier1, [onActivation1, onActivation2]],
        [serviceIdentifier2, [onActivation3]]
      ]),
    ...
    expect(cloneModule1Handlers.onDeactivations.getMap()).toStrictEqual(
      new Map([
        [serviceIdentifier1, [onDeactivation1, onDeactivation2]],
        [serviceIdentifier2, [onDeactivation3]],
      ]),
    ...
    expect(cloneModule2Handlers.onActivations.getMap()).toStrictEqual(
      new Map([[serviceIdentifier1, [onActivationMod2]]]),
    );

    expect(cloneModule2Handlers.onDeactivations.getMap()).toStrictEqual(
